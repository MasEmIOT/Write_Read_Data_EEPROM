...
void UART_ReadWrite_EEPROM()
{
	if(strcmp(rx_buf,"Write number") == 0)
	{
		...
		while(_number[j] != '.')
		{
			_number2[j] = _number[j];
			j++;
		}
		j = 0;
		LCD_Sent_String(_number2);
		LCD_Put_Cur(1,0);
		UART_Send_MultiByte(_number2,strlen(_number2));
		UART_Send_Byte('\n');
		...
		while(_address[j] != '*')
		{
			_address2[j] = _address[j];
			j++;
		}
		j = 0;
		LCD_Sent_String(_address2);
		UART_Send_MultiByte(_address2,strlen(_address2));
		Write_Number_EEPROM(atoi(_number2),atoi(_address2));	
		UART_Send_Byte('\n');
		rx_index = 0;
		strcpy(rx_buf,"");
		//rx_buf[rx_index] = '\0';
...
//TIM1_delay(10000);
		...
		LCD_Sent_String(_address2);
		UART_Send_MultiByte(_address2,strlen(_address2));
		Write_Character_EEPROM(_character,atoi(_address2));	
		UART_Send_Byte('\n');
		rx_index = 0;
		strcpy(rx_buf,"");
		//rx_buf[rx_index] = '\0';
	...
	if(strcmp(rx_buf,"Write string") == 0)
	{
		...
		while(_string[j] != '.')
		{
			_string2[j] = _string[j];
			j++;
		}
		j = 0;
		LCD_Put_Cur(1,0);
		LCD_Sent_String(_string2);
		UART_Send_MultiByte(_string2,strlen(_string2));
		LCD_Put_Cur(0,0);
		LCD_Sent_String("At address: ");
		UART_Send_Byte('\n');
		UART_Send_MultiByte("At address: ",12);
		...
		LCD_Sent_String(_address2);
		UART_Send_MultiByte(_address2,strlen(_address2));
		Write_String_EEPROM(_string2,atoi(_address2));	
		UART_Send_Byte('\n');
		rx_index = 0;
		strcpy(rx_buf,"");
	}
	...
	if(strcmp(rx_buf,"Read string") == 0)
	{
		...
		while(_length[j] != '.')
		{
			_length2[j] = _length[j];
			j++;
		}
		j = 0;
		LCD_Clear();
		LCD_Sent_String(_length2);
		UART_Send_MultiByte(_length2,strlen(_length2));
		rx_index = 0;
		UART_Send_Byte('\n');
    //int length = atoi(_length);
	
		for(int i = 0; i < atoi(_length2); i++)
		{
			//Read_Data(atoi(_address)+i);
			Read_Data(atoi(_address2)+i);
			_character = Read_Character_EEPROM();
